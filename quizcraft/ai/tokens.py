...
def estimate_token_count(text: str) -> int:
    ...
    This is a rough approximation, but useful for ensuring we don't exceed limits.
    ...
    estimated_tokens = (
        ...
        (whitespace_count / CLAUDE_AVG_CHARS_PER_TOKEN * CLAUDE_WHITESPACE_MULTIPLIER) +
        (special_char_count / CLAUDE_AVG_CHARS_PER_TOKEN * CLAUDE_SPECIAL_CHAR_MULTIPLIER)
    )
...
def optimize_context(text: str, max_tokens: int = 100000) -> str:
    ...
    middle_ratio = (max_tokens - estimate_token_count('\n\n'.join(beginning + ending))) / estimated_tokens
    if middle_ratio > 0:
        ...
        optimized_text = '\n\n'.join(beginning + ["..."] + sampled_middle + ["..."] + ending)
    else:
