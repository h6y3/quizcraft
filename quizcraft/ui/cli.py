...
def create_parser() -> argparse.ArgumentParser:
    ...
    # Generate command
    generate_parser = subparsers.add_parser("generate", help="Generate questions from PDF")
    ...
    generate_parser.add_argument(
        "--topic", "-t",
        help="Focus on a specific topic (optional)"
    )
    generate_parser.add_argument(
        "--disable-ocr", 
        action="store_true", 
...
def handle_extract_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        try:
            result = pdf_service.extract_text(args.pdf_path, segment=args.segment)
        ...
        if not result["success"]:
            ...
            print(f"Error: Failed to extract text from {args.pdf_path}: {error_reason}", file=sys.stderr)
            return 1
        ...
        if args.output:
            ...
            # Print a summary instead of the full JSON for better terminal output
            if args.segment:
                print(f"Segmented into {len(result['segments'])} segments")
                for i, seg in enumerate(result['segments'][:3]):  # Show the first 3 segments
                    preview = seg["text"][:100].replace("\n", " ")
                    print(f"Segment {i+1} ({seg.get('type', 'unknown')}): {preview}...")
                if len(result['segments']) > 3:
                    print(f"... and {len(result['segments']) - 3} more segments")
            else:
                text_preview = result["text"][:200].replace("\n", " ")
                print(f"Extracted text ({len(result['text'])} characters): {text_preview}...")
...
def handle_generate_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.topic:
            ...
            context_result = pdf_service.extract_topic_context(args.pdf_path, args.topic)
            if not context_result["success"]:
                print(f"Error: Failed to extract text from {args.pdf_path}", file=sys.stderr)
                return 1
        ...
        # Generate questions
        print(f"Generating {args.num_questions} questions at {args.difficulty} difficulty...")
