...
class ClaudeClient:
    ...
    def __init__(
        ...
        Args:
            api_key: Optional API key\
                . If not provided, will be retrieved from env.
            model: Claude model to use\
                . Defaults to claude-3-haiku for cost efficiency.
        """
    ...
    def generate_response(
        ...
        Args:
            ...
            stop_sequences: Optional list\
                of sequences that will stop generation
        Returns:
    ...
    def _validate_input(self, prompt: str) -> None:
        ...
        if token_estimate > 100000:  # Claude 3 models support up to 200K
    ...
    def _handle_rate_limit_error(self, attempt: int, error: Exception) -> None:
        ...
        logger.warning(
            f"Rate limit hit. Retrying in {delay:.1f}s. Error: {str(error)}"
        )
        time.sleep(delay)
    ...
    def _handle_api_error(self, attempt: int, error: Exception) -> bool:
        ...
        if attempt < MAX_RETRIES - 1:
            delay = self._calculate_delay(attempt)
            logger.warning(
                f"API error: {str(error)}. Retrying in {delay:.1f}s."
            )
            time.sleep(delay)
            return True
        else:
    ...
    def _handle_network_error(self, attempt: int, error: Exception) -> bool:
        ...
        if attempt < MAX_RETRIES - 1:
            delay = self._calculate_delay(attempt)
            logger.warning(
                f"Network error: {str(error)}. Retrying in {delay:.1f}s."
            )
            time.sleep(delay)
            return True
        else:
            logger.error(
                f"Network error after {MAX_RETRIES} attempts: {str(error)}"
            )
            raise RuntimeError(
