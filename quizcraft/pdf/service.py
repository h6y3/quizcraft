...
class PDFService:
    ...
    def extract_text(self, pdf_path: str, segment: bool = False) -> Dict[str, Any]:
        """
        ...
        if not raw_segments:
            ...
            return {"pdf_path": pdf_path, "text": "", "segments": [], "success": False}
    ...
    def extract_topic_context(self, pdf_path: str, topic: str) -> Dict[str, Any]:
        """
        ...
        if relevant_segments:
            logger.info(f"Found {len(relevant_segments)} segments mentioning '{topic}'")
            combined_text = "\n\n".join([seg.get("text", "") for seg in relevant_segments])
            return {
                "pdf_path": pdf_path,
                "topic": topic,
                "text": combined_text,
            ...
            logger.warning(f"No segments found containing '{topic}'. Using full text.")
