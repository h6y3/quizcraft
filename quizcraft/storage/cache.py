...
class ResponseCache:
    ...
    def get(
        ...
        try:
            ...
            logger.warning(
                f"Failed to parse cached response. Hash: {hash_key}"
            )
            return None
    ...
    def delete(self, prompt: str, params: Dict[str, Any]) -> bool:
        ...
        cursor.execute(
            "DELETE FROM response_cache WHERE hash = ?", (hash_key,)
        )
    ...
    def get_stats(self):
        ...
        if min_timestamp and max_timestamp:
            oldest = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime(min_timestamp)
            )
            newest = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime(max_timestamp)
            )
        else:
            oldest = newest = "N/A"
